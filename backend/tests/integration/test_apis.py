...
def test_get_default_admin_token(api_proxy_url, docker_services):
    token_header = _get_access_token_header(api_proxy_url, ADMIN_EMAIL, ADMIN_PASSWORD)
    assert token_header is not None


def test_api_users(api_proxy_url, docker_services):
    # Signup test_user
    response = requests.post(f"{api_proxy_url}/users/signup", json=test_user.model_dump(), timeout=20)
    assert response.status_code == 200
    assert response.json()["email"] == test_email

    # Get token header for test_user
    header = _get_access_token_header(api_proxy_url, test_email, test_password)
    assert header is not None

    # Get current user from api
    response = requests.get(f"{api_proxy_url}/users/me", headers=header, timeout=20)
    assert response.status_code == 200
    assert response.json()["email"] == test_email
    ...
    response = requests.patch(f"{api_proxy_url}/users/me",
    ...
    response = requests.get(f"{api_proxy_url}/users/{updated_user.id}", headers=header, timeout=20)
    assert response.status_code == 200
    get_user = UserPublic.model_validate_json(response.text)
    assert get_user.email == new_email

    # Delete test_user
    response = requests.delete(f"{api_proxy_url}/users/me", headers=header, timeout=20)
    assert response.status_code == 200

    # Make sure test_user was deleted
    response = requests.delete(f"{api_proxy_url}/users/me", headers=header, timeout=20)
    assert response.status_code == 404
    response = requests.delete(f"{api_proxy_url}/users/{updated_user.id}", headers=header, timeout=20)
    assert response.status_code == 404


def test_api_pdfs(api_proxy_url, docker_services):
    header = _get_access_token_header(api_proxy_url, ADMIN_EMAIL, ADMIN_PASSWORD)
    with open("backend/tests/api/test.pdf", "rb") as pdf_file:
        # Upload test pdf file
        files = {"pdf_file": pdf_file}
        response_post = requests.post(f"{api_proxy_url}/pdfs", files=files, headers=header, timeout=20)
        ...
        response_get = requests.get(f"{api_proxy_url}/pdfs/{db_pdf.id}", headers=header, timeout=20)
        assert response_get.status_code == 200

        # remove uploaded test file
        response_delete = requests.delete(f"{api_proxy_url}/pdfs/{db_pdf.id}.pdf", headers=header, timeout=20)
        assert response_delete.status_code == 200
        # Make sure file was removed
        response_after_delete = requests.get(f"{api_proxy_url}/pdfs/{db_pdf.id}.pdf", headers=header, timeout=20)
        assert response_after_delete.status_code == 404

        # remove pdf database entry
        response_delete = requests.delete(f"{api_proxy_url}/pdfs/{db_pdf.id}", headers=header, timeout=20)
        assert response_delete.status_code == 200
        # Make sure entry was removed
        response_after_delete = requests.get(f"{api_proxy_url}/pdfs/{db_pdf.id}", headers=header, timeout=20)
        assert response_after_delete.status_code == 404
