...
import requests
import os
from backend.app.models import UserCreate, PdfPublic
...
def test_signup(api_url, docker_services):
...
def test_api_pdfs(api_url, docker_services):
    ...
    with open("backend/tests/api/test.pdf", "rb") as pdf_file:
        ...
        response_post = requests.post(f"{api_url}/pdfs", files=files, headers=token_header, timeout=20)
        ...
        pdf_file.seek(0)
        response_post_again = requests.post(f"{api_url}/pdfs", files=files, headers=token_header, timeout=20)
        assert response_post_again.status_code == 409

        # Get uploaded test file from backend and compare contents with original
        response_get = requests.get(f"{api_url}/pdfs/{db_pdf.id}.pdf", headers=token_header, timeout=20)
        pdf_file.seek(0)
        ...
        response_get = requests.get(f"{api_url}/pdfs/{db_pdf.id}", headers=token_header, timeout=20)
        assert response_get.status_code == 200

        # remove uploaded test file
        response_delete = requests.delete(f"{api_url}/pdfs/{db_pdf.id}", headers=token_header, timeout=20)
        assert response_delete.status_code == 200

        # Make sure file was removed
        response_after_delete = requests.get(f"{api_url}/pdfs/{db_pdf.id}.pdf", headers=token_header, timeout=20)
        assert response_after_delete.status_code == 404
        response_after_delete = requests.get(f"{api_url}/pdfs/{db_pdf.id}", headers=token_header, timeout=20)
        assert response_after_delete.status_code == 404